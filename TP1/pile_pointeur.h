...
PtPile* Inserer_BonPlace_PPile(PtPile *mapile, PtPile *NE)
{
	...
	while ((mapile) && ((Sommet_PPile(*mapile)<NE->Entier)))
	{
...
PtPile *Supprimer_1ere_Occ(PtPile *mapile, int val)
{
    PtPile *tmp, *pcrt;
	...
	while (!(Est_Vide_PtPile(mapile)) && (Sommet_PPile(*mapile) != val))
	{
		pcrt = mapile;
		mapile = mapile->svt;
		pcrt->svt = tmp;
	...
	if (Est_Vide_PtPile(mapile))
		...
		printf("\nL'?l?ment ? supprimer n'existe pas dans la pile");
	
	else
 	
 		mapile=depiler(mapile);
 	
	//Restaurer la pile ? son ?tat initial	
	while (!Est_Vide_PtPile(tmp))
	{
		pcrt = tmp;
		tmp = tmp->svt;
