...
//fonction qui permet d'inserer un element a la fin de la liste 
PtListe *trie_par_insertion(PtListe *maliste)
{
   PtListe *tmp,*crt,*trier;
   crt=maliste;
   trier=maliste;
   tmp=crt->svt;
   while(tmp)
   {
   		if(trier->Entier<=tmp->Entier)
   		{
   			trier=trier->svt;
   			tmp=tmp->svt;
		}
		else if(crt->Entier<tmp->Entier)
		{
			trier=tmp->svt;
			tmp->svt=maliste;
			maliste=tmp;
...
//			tmp=trier->svt;
		...
		{
			while((crt->svt->Entier<=tmp->Entier) && crt!=trier) crt=crt->svt;
			trier=tmp->svt;
			tmp->svt=crt->svt;
			crt->svt=tmp;
			trier=trier->svt;
