...
void AffichageHorizental(Noeud *Arb, int n) 
{
    if (Arb) {
        ...
        if (Arb->info.operateur != '\0') {
            ...
            // Correction ici : utiliser Arb->info.operande au lieu de Arb->info.valeur
        }
...
int main() {
    ...
    printf("\nAffichage de l'expression arithmétique en notation infixée :\n");
    AffichageHorizental(arbre,0);

    float resultat = Evaluer_Arbre_Exp(arbre);
    printf("\n\nResultat de l'expression arithmétique : %.2f\n", resultat);

    // Vous pouvez également tester la version utilisant une pile
    // Cellule *pile_exp = Expression_Arithmetique_Pile();
    // float resultat_pile = Evaluer_Pile_Exp(pile_exp);
    // printf("\nResultat de l'expression arithmétique avec pile : %.2f\n", resultat_pile);
