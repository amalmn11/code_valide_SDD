...
/* **********
    Nom         : Transforme;
    Entrees     : double *operande : pour stoker la valeur de nombre extrait
    ...
    Description : la fonction permet d'extrait un nombre reel dans une chaine
                ...
                le nombre extrait. 
********** */
//char Transforme(double *operande)
//{
...
//                else result += (double)((double)(control)*pow(10,f--));
//        }
//        c = getchar();
...
//----> fonction pour sauter les blancs
char epurer_blanc()
{
	char carac;
...
float lire_reel(char *caractere)
{
	
	char c;
	int i,signe;
	float Ptdecim,Ptent;
	c=Epeurer_Blanc(c);//lire le premier caractere
	//traiter le signe
	...
	if(!signe) signe =1;
	if(c == RC) return ((float) (Ptent+Ptdecim)*signe);
	if(est_operateur(c))
	{
		*caractere=c;
		return ((float) (Ptent+Ptdecim)*signe);
	}
	printf("\nERROR");
	return ((float) -1);
}
...
Noeud *Construire_Arbre_Exp()
{
    ...
    double operande; // pour stocker l'operande extrait
    ...
    while ((operande = lire_reel(&operateur)) != RC)
    {   
...
/* **********
    Nom         : Calculer_Exp;
    Entrees     : double op1      : le premier operande 
                  char operateur : un operateur 
                  double op2      : le deuxieme operande 
    Sorties     : double result   : le resultat de l'operation 
...
/* **********
    ...
    Sorties     : double      : le resultat d'evaluation de l'expression 
                arithmetique 
...
double Evaluer_Arbre_Exp(Noeud *Arb)
{
    if (Arb)
    {
        if (!Arb->fgc && !Arb->fdt) return ((double)Arb->info.operande);
        return ((double)Calculer_Exp(Evaluer_Arbre_Exp(Arb->fgc),
                                    Arb->info.operateur,
...
********** */
double Evaluer_Pile_Exp(Cellule *pile)
{
    ...
    while (tmp->svt) tmp = Evaluer_Operation_Pile(tmp);
    return ((double)tmp->info.operande);
}
...
int main() {
    ...
    double resultat_arbre = Evaluer_Arbre_Exp(Arbre);
    ...
    double resultat_pile = Evaluer_Pile_Exp(pile_expression);
