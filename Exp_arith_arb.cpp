...
int main() {
    Noeud *ArbreExpression;
    float resultat;

    // Appel de la fonction pour construire l'arbre d'expression
    ArbreExpression = Construire_Arbre_Exp();

    // Appel de la fonction pour évaluer l'arbre d'expression
    resultat = Evaluer_Arbre_Exp(ArbreExpression);

    // Affichage du résultat
    printf("\nLe resultat de l'expression est : %f\n", resultat);
    
    printf("\n\naffichage horizentale de l'arbre :");
    AffichageHorizental(ArbreExpression,0);
